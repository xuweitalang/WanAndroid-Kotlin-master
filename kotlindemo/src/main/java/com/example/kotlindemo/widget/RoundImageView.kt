...
class RoundImageView : AppCompatImageView {
    ...
    private fun readAttrs(context: Context, attrs: AttributeSet?) {
        ...
        borderColor =
            typedArray.getColor(R.styleable.RoundImageView_borderColor, 0xffff0000.toInt())
        borderWidth =
            typedArray.getDimensionPixelSize(R.styleable.RoundImageView_borderWidth, 0).toFloat()
        typedArray.recycle()
    }
...
//        canvas?.restore()
        ...
        if (borderWidth != 0f) {
            if (shape == 0) {
                canvas?.drawCircle(
                    centerX,
                    centerY,
                    Math.min(mHeight, mWidth) / 2f - borderPaint.strokeWidth / 2f,
                    borderPaint
                )
            } else if (shape == 1) {
                canvas?.drawRoundRect(shapeRectF, centerX, centerY, borderPaint)
            }
    ...
    private fun getBitmap(): Bitmap {
        ...
        if (shape == 0) {
            canvas.drawCircle(centerX, centerY, Math.min(mHeight, mWidth) / 2f, shapePaint)
        } else if (shape == 1) {
